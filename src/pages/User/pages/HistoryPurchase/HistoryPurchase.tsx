...
export default function HistoryPurchase() {
  ...
  const purchaseTabsLink = purchaseTabs.map((tab) => (
    ...
    >
      {tab.status === 0
        ? t('user details.all')
        : tab.status === 1
        ? t('user details.waitForConfirmation')
        : tab.status === 2
        ? t('user details.waitForGetting')
        : tab.status === 3
        ? t('user details.inProgress')
        : tab.status === 4
        ? t('user details.delivered')
        : t('user details.cancelled')}
    </Link>
  ));
  ...
  return (
    <div>
      <Helmet>
        <title>{'Lịch sử đơn hàng'} | Đạt Nguyễn Shop</title>
        <meta name='description' content={'Lịch sử đơn hàng của bạn'} />
      </Helmet>
            ...
            {purchasesInCart?.map((purchase) => (
              <div key={purchase._id} className='mt-4 rounded-sm border-black/10 bg-white p-6 text-gray-800 shadow-sm'>
                ...
                <div className='flex justify-end'>
                  <div>
                    <span>{t('user details.totalPrice')}</span>
